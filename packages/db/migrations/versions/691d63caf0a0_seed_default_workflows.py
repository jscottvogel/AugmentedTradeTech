...
Revision ID: 691d63caf0a0
Revises: 7c2d33a839c2
Create Date: 2026-05-20 16:23:34.180613
...
revision: str = '691d63caf0a0'
down_revision: Union[str, Sequence[str], None] = '7c2d33a839c2'
...
def upgrade() -> None:
    """Upgrade schema."""
    config_json = json.dumps(DEFAULT_WORKFLOW_CONFIG)
    connection = op.get_bind()
    connection.execute(
        sa.text("UPDATE companies SET workflow_config = :config WHERE workflow_config IS NULL OR workflow_config = '{}'::jsonb"),
        {"config": config_json}
    )
...
def downgrade() -> None:
    ...
    connection = op.get_bind()
    connection.execute(
        sa.text("UPDATE companies SET workflow_config = '{}'::jsonb")
    )
